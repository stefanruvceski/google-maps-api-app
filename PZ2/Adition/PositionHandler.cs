    ...
    public static class PositionHandler
    {
        public static NetworkModel DoParse(NetworkModel networkModel)
        {
            ...
            for (int i = 0; i < networkModel.vertices.Count; i++)
            {
                ...
                {
                    double latitude, longitude;
                    ToLatLon(networkModel.vertices[i].Vertices[j].X, networkModel.vertices[i].Vertices[j].Y, 34, out latitude, out longitude);
                    networkModel.vertices[i].Vertices[j].X = latitude;
                    networkModel.vertices[i].Vertices[j].Y = longitude;
                }
            }
